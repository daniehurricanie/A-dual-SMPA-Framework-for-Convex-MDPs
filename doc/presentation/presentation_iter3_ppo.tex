...
\definecolor{ppocolor}{RGB}{128,0,128} % purple for PPO replacements
\newcommand{\ppo}[1]{\textcolor{ppocolor}{#1}} % shorthand for PPO text
...
\begin{itemize}
  ...
  \item ``We'll compare this `Dual--SPMA' recipe against a strong baseline called \sout{NPG--PD, which is a natural policy-gradient primal--dual method} \ppo{PPO, which is a widely used clipped policy-gradient method}.''
\end{itemize}
...
%----------------------------------------------------------------------------
\slide{21}{Related Work: \sout{NPG--PD} \ppo{PPO / TRPO Style Methods} (Our Baseline)}
\speaker{Shervin}

\begin{itemize}
  \item ``Our main baseline is the \sout{\textbf{Natural Policy Gradient Primal--Dual} method, or NPG--PD, from Ding et al.\ 2020} \ppo{\textbf{Proximal Policy Optimization}, or PPO, from Schulman et al.\ 2017}.''
  
%   \item \sout{``NPG--PD is designed for constrained MDPs with a Lagrangian of the form:
%   \[
...
%   \pron{L of pi, lambda equals J-r of pi plus lambda times open-paren J-c of pi minus tau close-paren, with lambda greater-than-or-equal-to zero}}''  
  \item ``\ppo{PPO is a clipped surrogate trust-region style policy gradient method. It uses:}
  \ppo{\[
  L^{CLIP}(\theta) = \mathbb{E}\left[\min\left(r_t(\theta)\hat{A}_t, \text{clip}(r_t(\theta), 1-\epsilon, 1+\epsilon)\hat{A}_t\right)\right]
  \]
  where $r_t(\theta)$ is the probability ratio between new and old policies.}''
  
  \item ``\sout{On the \textbf{primal side}, it performs \textbf{natural policy gradient ascent} on the policy, using the Fisher information matrix as a geometry.}''
  
  \item ``\ppo{PPO uses a \textbf{clipped surrogate loss} that keeps the policy close to the previous one---similar in spirit to SPMA's `stay close' regularizer.}''
  
  \item ``\sout{On the \textbf{dual side}, it updates the Lagrange multiplier using:
  \[
  ...
  \item ``\ppo{Key properties of PPO:}''
  \begin{itemize}
    \item \ppo{``Very strong empirical performance---the de-facto standard in deep RL.''}
    \item \ppo{``Stable and easy to tune.''}
    \item \ppo{``We adapt PPO to our convex objective (entropy-reg or CMDP Lagrangian).''}
  \end{itemize}
  
  \item ``\sout{NPG--PD comes with theoretical guarantees: they show an $\mathcal{O}(1/\sqrt{T})$ \pron{O of one over square-root-T} bound on optimality gap and constraint violation.}''
  
  \item ``\sout{Because it's both principled and relatively strong empirically, we use NPG--PD as our \textbf{comparison point} for Dual--SPMA.}''
  
  \item ``\ppo{Because PPO is widely used and extremely strong empirically, we use it as our \textbf{comparison point} for Dual--SPMA.}''
\end{itemize}
...
\begin{itemize}
  ...
  \item ``Next I'll describe what we actually built: our \textbf{Dual--SPMA method}, occupancy estimators, and the \sout{NPG--PD} \ppo{PPO} baseline.''
\end{itemize}
...
\begin{itemize}
  ...
  \item ``Third, we implemented a \sout{\textbf{faithful NPG--PD baseline}} \ppo{\textbf{faithful PPO baseline on the same convex objective}} so that we could compare on equal footing.''
  
  \item ``Finally, we performed an \textbf{empirical comparison} of SPMA vs \sout{NPG--PD} \ppo{PPO} on constrained safety tasks, focusing on convergence and constraint satisfaction.''
\end{itemize}
...
\begin{itemize}
  ...
  \item ``\sout{SPMA and NPG--PD have the \textbf{same dual update}---the only difference is Step 3: SPMA loss vs.\ natural gradient.}''
  
  \item ``\ppo{SPMA and PPO have the \textbf{same dual update}---the only difference is Step 3: SPMA loss vs.\ PPO's clipped surrogate.}''
...
\begin{itemize}
  ...
  \item ``Notice that this dual update is the same as in \sout{NPG--PD} \ppo{PPO}; \textbf{the main difference is the policy optimizer}: SPMA vs \sout{natural gradient} \ppo{clipped surrogate}.''
\end{itemize}
...
\begin{itemize}
  \item ``We've now described the Dual--SPMA method, the occupancy estimators, and the \sout{NPG--PD} \ppo{PPO} baseline.''
...
\begin{itemize}
  ...
  \item ``We compare two methods: our \textbf{Dual--SPMA} and the \sout{\textbf{NPG--PD}} \ppo{\textbf{PPO}} baseline.''
...
\begin{itemize}
  ...
  \item ``Both SPMA and \sout{NPG--PD} \ppo{PPO} behave similarly here; they're both optimizing the same entropy-regularized objective.''
\end{itemize}
...
\begin{itemize}
  ...
  \item ``We see that Dual--SPMA and \sout{NPG--PD} \ppo{PPO} reach similar final performance, though their learning curves differ slightly.''
\end{itemize}

%----------------------------------------------------------------------------
\slide{33}{Results: Dual--SPMA vs \sout{NPG--PD} \ppo{PPO}}
\speaker{Ahmed}

\begin{itemize}
  \item ``Now let's compare Dual--SPMA against \sout{NPG--PD} \ppo{PPO} on the same constrained task.''
  ...
  \item ``\sout{NPG--PD is often smoother but requires careful tuning of the step size $\beta$ \pron{beta}.}''
  
  \item ``\ppo{PPO is stable and widely used; SPMA sometimes converges faster but is less standard.}''
  
  \item ``Overall, Dual--SPMA is competitive with \sout{NPG--PD} \ppo{PPO} on these tasks, which supports our claim that SPMA is a viable policy optimizer in the convex-MDP framework.''
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item ``a strong \sout{NPG--PD} \ppo{PPO} baseline,''
    \item ``and several occupancy estimators.''
  \end{itemize}
  
  \item ``In our experiments, Dual--SPMA is stable, learns to satisfy constraints, and performs competitively with \sout{NPG--PD} \ppo{PPO} on a tabular CMDP.''
